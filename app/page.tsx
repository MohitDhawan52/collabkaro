...
export default function HomePage() {
  const stats = [
    { label: 'Active Brands', value: '2,400+', color: 'stat-purple' },
    { label: 'Influencers', value: '18,000+', color: 'stat-green' },
    { label: 'Deals Closed', value: '94,000+', color: 'stat-yellow' },
    { label: 'Paid Out', value: '₹12Cr+', color: 'stat-blue' },
  ]

  const features = [
    { title: 'Smart Discovery', desc: 'Find the perfect influencer match with advanced filters by niche, audience size, engagement rate, and location.' },
    { title: 'Digital Agreements', desc: 'Create, sign, and store collaboration agreements in minutes. No PDFs, no lawyers needed for standard deals.' },
    { title: 'Secure Payments', desc: 'Escrow-protected payments ensure brands only pay when work is delivered and approved.' },
    { title: 'Live Analytics', desc: 'Track campaign performance, reach, and ROI in a single dashboard. No more spreadsheets.' },
    { title: 'Dispute Resolution', desc: 'Built-in mediation for when things go sideways. Fair outcomes for both sides.' },
    { title: 'Real-time Alerts', desc: 'Stay on top of every deal with instant notifications for messages, payments, and milestones.' },
  ]
    ...
    <div style={{ background: '#f0f2f7', minHeight: '100vh' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 18, color: '#1a1d23' }}>CollabSphere</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <Link href="/auth/login" style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e5e7eb', color: '#374151', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>Log in</Link>
      ...
      <section style={{ padding: '80px 40px 60px', maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#ede9fe', padding: '6px 14px', borderRadius: 20, marginBottom: 24 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: '#7c3aed' }}>NOW IN BETA — FREE TO JOIN</span>
        </div>
        <h1 style={{ fontSize: 'clamp(36px, 5vw, 60px)', fontWeight: 800, color: '#1a1d23', lineHeight: 1.15, marginBottom: 20 }}>
          Where Brands Meet<br />
          <span className="gradient-text">The Right Creators</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link href="/discover" style={{ padding: '14px 28px', borderRadius: 10, background: '#fff', color: '#374151', textDecoration: 'none', fontSize: 16, fontWeight: 600, border: '1px solid #e5e7eb' }}>Browse creators</Link>
        </div>
      </section>
      ...
      {/* Features */}
      <section style={{ padding: '0 40px 60px', maxWidth: 1000, margin: '0 auto' }}>
        <h2 style={{ fontSize: 30, fontWeight: 700, color: '#1a1d23', textAlign: 'center', marginBottom: 8 }}>Everything you need to collaborate</h2>
        <p style={{ color: '#9ca3af', textAlign: 'center', marginBottom: 40 }}>From discovery to payment — handled.</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {features.map((f) => (
            <div key={f.title} style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8, color: '#1a1d23' }}>{f.title}</div>
      ...
      <section style={{ padding: '0 40px 60px', maxWidth: 900, margin: '0 auto' }}>
        <h2 style={{ fontSize: 30, fontWeight: 700, color: '#1a1d23', textAlign: 'center', marginBottom: 8 }}>Three steps to your next campaign</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
          {steps.map((s) => (
            <div key={s.num} style={{ textAlign: 'center', padding: '32px 24px', background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 48, fontWeight: 800, color: '#7c3aed', opacity: 0.2, lineHeight: 1, marginBottom: 16 }}>{s.num}</div>
              <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8, color: '#1a1d23' }}>{s.title}</div>
      ...
      {/* Testimonials */}
      <section style={{ padding: '0 40px 60px', maxWidth: 1000, margin: '0 auto' }}>
        <h2 style={{ fontSize: 30, fontWeight: 700, color: '#1a1d23', textAlign: 'center', marginBottom: 40 }}>Creators and brands love CollabSphere</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {testimonials.map((t) => (
            <div key={t.name} style={{ background: '#fff', borderRadius: 12, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14, color: '#1a1d23' }}>{t.name}</div>
      ...
      {/* Footer */}
      <footer style={{ borderTop: '1px solid #e5e7eb', padding: '24px 40px', textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>
