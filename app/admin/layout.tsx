...
const NAV_ITEMS = [
  ...
  { href: '/admin/brands',      label: 'Brands',         icon: Briefcase,       iconBg: 'rgba(249,115,22,0.5)',  iconColor: '#fff' },
  { href: '/admin/influencers', label: 'Influencers',    icon: Users,           iconBg: 'rgba(168,85,247,0.5)', iconColor: '#fff' },
  { href: '/admin/gigs',        label: 'All Gigs',       icon: Settings,        iconBg: 'rgba(6,182,212,0.5)',  iconColor: '#fff' },
  { href: '/admin/collabs',     label: 'Collaborations', icon: GitMerge,        iconBg: 'rgba(16,185,129,0.5)', iconColor: '#fff' },
  { href: '/admin/payments',    label: 'Payments',       icon: Wallet,          iconBg: 'rgba(234,179,8,0.55)',  iconColor: '#fff' },
  { href: '/admin/stats',       label: 'Platform Stats', icon: BarChart3,       iconBg: 'rgba(236,72,153,0.5)', iconColor: '#fff' },
  { href: '/admin/notify',      label: 'Notifications',  icon: Bell,            iconBg: 'rgba(239,68,68,0.5)',  iconColor: '#fff' },
]
  ...
  return (
    <Link
      ...
      style={{
        ...
        color: active ? '#fff' : hovered ? '#fff' : 'rgba(255,255,255,0.85)',
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(145deg, #eef2ff 0%, #dbeafe 50%, #ecfdf5 100%)' }}>
      <aside style={{ width: 230, flexShrink: 0, background: 'linear-gradient(160deg, #0c1445 0%, #1d4ed8 55%, #06b6d4 100%)', display: 'flex', flexDirection: 'column', padding: '20px 12px', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '4px 8px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 800, color: '#fff', fontFamily: 'Plus Jakarta Sans, sans-serif', letterSpacing: -0.3 }}>CollabKaro Admin</span>
        </div>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 3, flex: 1 }}>
          {NAV_ITEMS.map((item) => (
            <NavItem key={item.href} item={item} active={pathname === item.href} onClick={() => setMobileNavOpen(false)} />
          ))}
        </nav>
